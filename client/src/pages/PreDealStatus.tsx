...
export default function PreDealStatus() {
  ...
  const predealQuery = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return TERMINAL_STATUSES.has(status as PredealStatus) ? false : 4000;
    },
  });
  ...
  const statusClass = STATUS_STYLES[predeal.status as PredealStatus] ?? "border-border/60 bg-secondary/40 text-muted-foreground";
